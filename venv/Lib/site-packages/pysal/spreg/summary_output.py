...
def ML_Lag_BC(reg, w, vm, spat_diag, regimes=False, LM=False):  # extra space d
    ...
    if LM:
        ...
        reg.__summary['summary_r2'] += "                                                 %-22s:%12.3f (%1.3f)\n" % (
            'LM cond. rho 0 and lambda unknown', reg.LM_C_R0_LU, chi2.sf(reg.LM_C_R0_LU, df3))
        reg.__summary['summary_r2'] += "                                                 %-22s:%12.3f (%1.3f)\n" % (
            'LM cond. rho unkown and lambda 0', reg.LM_C_RU_L0, chi2.sf(reg.LM_C_RU_L0, df4))
        reg.__summary['summary_r2'] += "                                                 %-22s:%12.3f (%1.3f)\n" % (
            'LM cond. rho unknown and lambda 1', reg.LM_C_RU_L1, chi2.sf(reg.LM_C_RU_L1, df5))
    # build coefficients table body
    summary_coefs_allx(reg, reg.z_stat)
    if regimes:
        summary_regimes(reg)
