...
class BaseML_Lag_BC(RegressionPropsY, RegressionPropsVM):
    ...
    def __init__(self, y, x, z, w, method='full', epsilon=0.0000001, LM=False):
        ...
        if (np.diag(self.vm1) <= 0).any():
            self.vm1 = self.grad_vect(W, b, self.ybc, res.x[0], res.x[1])
    ...
    def get_pred_y(self, w, b, rho, lam, epsilon):
        ...
        try:
            ...
            ybc = inverse_prod(
                w.sparse, xb, rho, inv_method="true_inv", threshold=epsilon)
        ybc[ybc < 0] = 0.0001
    ...
    def grad_vect(self, W, b, ybc, rho, lam, case=0):
        ...
        if case == 0:
            ...
            wy = spdot(W, ybc)
            f1 = spdot(self.xfull.T, e) / self.sig2
            f2 = - tr1 + spdot(e.T, wy) / self.sig2
            f3 = np.log(self.y).sum() - spdot(e.T, ac) / self.sig2
            f4 = -self.n / (2.0 * self.sig2)
            ...
            # Conditional test of rho = 0 and lam estimated
                ...
                f3 = -self.z / np.sqrt(self.sig2)
                f4 = -spdot(W, lny) / np.sqrt(self.sig2)
                f5 = (cy - spdot(cx, b[1:xsize+1])) / np.sqrt(self.sig2)
    ...
    def inf_mat(self, W, w, b, ybc, pred_y, pred_ylam, pred_ybc, rho, lam):
        ...
        acy = spdot(a, cy)
        cxb = spdot(cx, b[1:xsize+1])
        dcxdlb = spdot(dcxdl, b[1:xsize + 1])
        ac = acy - cxb
        adcy = spdot(a, dcydl)
        adc = adcy - dcxdlb
        ...
        cx1[:, 1:self.x.shape[1]+1] = spdot(e.T, cx) / self.sig2
